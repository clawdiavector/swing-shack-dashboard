...
## 2026-08-17T06:58Z — fix(ux): add standard <span class="sub"> to 6 sections missing the subtitle pattern

Pre-flight: working tree clean (after git stash of the nightly data/freshness.json cron churn — those are auto-regenerated by the nightly cron, not my work). Picked Priority #4 (weak UX: confusing/missing orientation copy).

Root cause: A Playwright probe of all 28 sections via go(sec) on the LIVE deployment found that six sections shipped without the standard <span class="sub"> subtitle line that every other section uses: sec-gbp, sec-reddit, sec-faqs, sec-seo, sec-agents, and sec-hashtagseo (which had a div.muted instead). The other ~22 sections consistently render <div class="section-h"><h2 ...>Title</h2><span class="sub">subtitle</span></div>. The six outliers landed on a bare H2 with nothing — clicking the nav items gave users no clue what to expect.

Fix (1 file, +6 / -10): campaign-os/campaign-os.html — for each of the 6 sections, added the standard <span class="sub"> immediately following the <h2>. sec-hashtagseo got the biggest change — its old div.muted block collapsed to one span.

Regression test (1 new file, +198 / -0): campaign-os/tests/test_v2026_08_17_section_subtitle_consistency.py — 10 tests covering each section has the new sub, no em-dashes, no fabricated numbers, every sub >30 chars and contains the expected fragment, and the h2/sub pattern is intact.

Verified (LIVE, authed, Playwright, post-deploy): all 6 expected sub-text fragments FOUND in the served bundle; all 10/10 new tests pass; Playwright screenshots show the new sub-label rendered under each H2; 0 page errors / 0 console errors / 0 failed network requests; /api/health green.

Next pick: ~26 EXPLAINERS blocks still reference 2026-07-30-era card names; the 8x 404s on /api/visual-library/.../*.jpg; Home dashboard renderRow missing-space bug for path+count rows.
...
## 2026-08-18T06:50Z — fix(library): approved tab 404s + search-everything tile silently no-ops

Pre-flight: working tree clean after stashing the nightly `data/freshness.json` + `data/weekly-report.md` cron churn (those are auto-regenerated by the nightly cron, not my work). Picked Priority #1 (broken browser flow: 404 on the "approved" Library tab + silent JS crash on the "Search everything" quick-launch tile).

Root cause: a Playwright probe of all 28 sections found two SPA-only bugs in the same `loadLibPane()` + `renderLibrary()` module in `campaign-os/campaign-os.html`:

**Bug 1 — "approved" tab permanent "Failed to load."**
The `loadLibPane(kind='approved')` branch called `API.get('/api/assets?status=approved&brand='+bid)`. The `/api/assets` GET endpoint doesn't exist on the backend (only PATCH/GET-single for `/api/assets/<id>` is wired), so the call 404s and the user sees a permanent "Failed to load." for every brand. Also the renderer read `a.title / a.kind / a.pillar` which never existed on the asset shape (always rendered as "undefined · undefined").

**Bug 2 — Search-everything quick-launch tile silently no-ops**
The `#lib-search-trigger` click handler had `$$('#lib-tabs .lib-tab').find(b => b.dataset.libTab === 'captions')`. `$$` returns a NodeList, not an Array; `.find` doesn't exist on a NodeList; the handler threw "$$(...).find is not a function" silently (caught by the surrounding try/catch in the caller), de-highlighted every tab, and left the pane stuck on the previous tab's content. The user clicked the Search tile, nothing happened, no feedback.

Fix (1 file, +24 / -7): `campaign-os/campaign-os.html`
- `loadLibPane(kind='approved')` → swap to `API.get('/api/intel/review_inbox?brand='+encodeURIComponent(bid))` which is the canonical source: backend already buckets into `{approved, pending, rejected}`. Render `a.name`, `a.campaignName`, `a.platform` (the real field names on this endpoint). Empty-state copy now surfaces the active brand so the user can see why the list is empty.
- `#lib-search-trigger` click handler → spread the NodeList into an Array first: `[...$$('#lib-tabs .lib-tab')].find(b => b.dataset.libTab === 'captions')`. Added 5-line comment explaining the NodeList gotcha for the next maintainer.

Regression test (1 new file, +123 / -0): `campaign-os/tests/test_v2026_08_18_library_approved_tab_fix.py` — 6 tests:
- Search-everything handler no longer contains `$$(...).find(`.
- Spread-then-find pattern is present.
- No other `$$().find(` call survives anywhere in the file (single regression-prone spot).
- Approved tab no longer references the broken `/api/assets` endpoint.
- Approved tab calls `/api/intel/review_inbox` and reads `r.approved`.
- Approved tab reads correct field names (a.name, a.campaignName, a.platform) and does NOT read a.title or a.pillar (which never existed on the asset shape and would render as "undefined").
- Approved tab does NOT client-side filter by `campaignId.startsWith(bid)` — the campaignId shape (e.g. `use-the-right-equipment-mq5l90bk`) does not start with the brand_id (`swing-shack`); a naive prefix-filter would drop legitimate Swing Shack assets.

Verified (LIVE, authed, Playwright, post-deploy):
- `curl https://swing-shack-dashboard-production.up.railway.app/api/health` → 200 `{"status":"ok"}`.
- Playwright walk on the LIVE Library:
  - **Approved tab (post-fix):** pane shows "Feed Post — The Fitting Solution — positioning professional fitting as the obvious answer · Use the Right Equipment · instagram". 1 thumbnail. No "Failed to load.".
  - **Search everything tile (post-fix):** Captions tab is `.on`, `#lib-search-input` is created and auto-focused, pane shows "Type to search.", typing "trackman" returns 30 results starting with "TrackMan Intelligence". No JS crash.
  - 0 console errors. 0 page errors. 0 failed network requests.
- 6/6 new tests pass; full suite: 271 failed (all pre-existing, /data read-only mount) / 987 passed (was 277/981 before my change: +6 new passes, no regressions).
- Screenshot: `/tmp/co-nightshift/walkthrough_lib_approved_20260818.png` + `walkthrough_lib_search_20260818.png`.

Standing rules honored: SPA-only patch (loadLibPane + the lib-search-trigger handler), 1 file changed; no API contract change; no publish/Postiz touched; no tokens in chat; branch stays on `feat/asset-state-engine`; atomic single commit (amended once after live verification found the prefix-filter was overzealous); no force push on the first push (force-with-lease only on the amend); no main; regression test added so the fix is CI-locked.

Next pick (from the prior nightshift log + new probe findings): the Home dashboard `renderRow` missing-space bug for path+count rows; the 26 EXPLAINERS blocks still referencing 2026-07-30-era card names (cheap text sweep, last cheap em-dash-risk lane); Billboard Lab "Visual briefs" pill mix has the same `object-fit:cover` cropping bug the meme strip had.

Learned: A NodeList returned from a `$`/`$$` helper looks like an Array in 99% of cases (it has `.length`, is iterable, can be spread). But it has no `.find` / `.filter` / `.map`. Spread-to-array (`[...nodeList]`) or explicit `Array.from(nodeList)` is the cheapest fix. Worth a `$$` typedef comment in the page or, better, a `function $$(sel){return Array.from(document.querySelectorAll(sel))}` to remove the entire class of bug across the file. The current file has 1 `$$().find` remaining (now fixed); no `$$().map` / `$$().filter` / `$$().some` were found in this pass but a future refactor that adds one would re-introduce the same silent failure mode. The right follow-up is to change `$$` to return an Array once and for all — separate tick.

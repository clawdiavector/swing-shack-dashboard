"""v2026-08-19 — Playwright walk of the LIVE Hook Bank section to confirm the
...
Run: cd campaign-os && ../.venv/bin/python3 tests/walk_hooks_sub_fix.py
"""
import os
import sys
import time
...
def login_and_walk(base_url):
    with sync_playwright() as p:
        ...
        # Make sec-hooks visible (sections are display:none by default; only .on shows).
        ...
        page.evaluate("""() => {
            ...
            const sec = document.getElementById('sec-hooks');
            if (sec) sec.classList.add('on');
        }""")
        page.wait_for_timeout(800)
        page.wait_for_timeout(1500)

        # Trigger renderHooks (the section was just scrolled into view; the
        # SPA lazy-renders per section when shown).
        render_attempt = page.evaluate("""async () => {
            if (typeof renderHooks === 'function') {
                try { await renderHooks(); return 'rendered'; } catch (e) { return 'err:' + e.message; }
            }
            return 'no renderHooks';
        }""")
        page.wait_for_timeout(1500)
        ...
        result = page.evaluate(
            """(render) => {
                ...
                const sub = document.getElementById('hooks-total');
        ...
        shot_path = SHOT_DIR / f"walkthrough_hooks_sub_{TS}.png"
        ...
        sec_shot = None
        if sec_box:
            sec_shot = SHOT_DIR / f"walkthrough_hooks_sub_section_{TS}.png"
            try:
                page.screenshot(
                    path=str(sec_shot),
                    clip={

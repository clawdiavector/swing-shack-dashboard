"""v2026-08-19 — Playwright walk of the LIVE Billboard Lab section to confirm
the new sub renders on first paint, carries id + data-help + data-help-title,
and matches the test's expected static fallback. Captures screenshot to
/tmp/co-nightshift/.

Run: cd campaign-os && ../.venv/bin/python3 tests/walk_billboard_sub_fix.py
"""
import os
import sys
import time
...
def login_and_walk(base_url):
    with sync_playwright() as p:
        ...
        # Make sec-billboards visible (sections are display:none by default;
        # only .on shows). This is what the SPA does when you click a nav
        # item. We do it manually so we can screenshot the section directly.
        page.evaluate("""() => {
            document.querySelectorAll('.section').forEach(s => s.classList.remove('on'));
            const sec = document.getElementById('sec-billboards');
            if (sec) sec.classList.add('on');
        }""")
        page.wait_for_timeout(800)
        page.wait_for_timeout(1500)

        # Trigger renderBillboards (the section was just scrolled into view;
        # the SPA lazy-renders per section when shown).
        render_attempt = page.evaluate("""async () => {
            if (typeof renderBillboards === 'function') {
                try { await renderBillboards(); return 'rendered'; } catch (e) { return 'err:' + e.message; }
            }
            return 'no renderBillboards';
        }""")
        page.wait_for_timeout(1500)
        ...
        result = page.evaluate(
            """(render) => {
                ...
                const sub = document.getElementById('bb-summary');
        ...
        shot_path = SHOT_DIR / f"walkthrough_billboard_sub_{TS}.png"
        ...
        sec_shot = None
        if sec_box:
            sec_shot = SHOT_DIR / f"walkthrough_billboard_sub_section_{TS}.png"
            try:
                page.screenshot(
                    path=str(sec_shot),
                    clip={

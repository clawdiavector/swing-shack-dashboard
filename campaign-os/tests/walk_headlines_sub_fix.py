"""v2026-08-19 — Playwright walk of the LIVE Headline Generator section to
confirm the new sub renders on first paint, carries id + data-help +
data-help-title, and matches the test's expected static fallback. Captures
a screenshot into /tmp/co-nightshift/.

Run: cd campaign-os && ../.venv/bin/python3 tests/walk_headlines_sub_fix.py
"""
import os
import sys
import time
...
def login_and_walk(base_url):
    with sync_playwright() as p:
        ...
        # Make sec-headlines visible. We also exercise the JS path that
        # sets #head-summary so the walker captures the post-render text too.
        page.evaluate("""() => {
            document.querySelectorAll('.section').forEach(s => s.classList.remove('on'));
            const sec = document.getElementById('sec-headlines');
        ...
        render_attempt = page.evaluate("""async () => {
            if (typeof renderHeadlines === 'function') {
                try { await renderHeadlines(); return 'rendered'; }
                catch (e) { return 'err:' + e.message; }
            }
            return 'no renderHeadlines';
        }""")
        ...
        result = page.evaluate(
            """(render) => {
                ...
                const sub = document.getElementById('head-summary');
        ...
        shot_path = SHOT_DIR / f"walkthrough_headlines_sub_{TS}.png"
        ...
        sec_shot = None
        if sec_box:
            sec_shot = SHOT_DIR / f"walkthrough_headlines_sub_section_{TS}.png"
            try:
                page.screenshot(
                    path=str(sec_shot),
                    clip={

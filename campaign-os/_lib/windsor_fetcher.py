"""
windsor_fetcher.py - the build_meta_ads + build_google_ads functions the
weekly-report brain calls to populate data/meta-ads.json + data/google-ads.json
with live Windsor.ai data.

Lives in _lib/ so it's importable from both:
  - the local Mac: `python3 scripts/fetch_windsor.py` (uses sys.path injection)
  - the live Railway deploy: `from _lib.windsor_fetcher import build_meta_ads`
    (called by POST /api/admin/windsor-refresh, no path-magic required)

REPLACES the synthesised data meta-ads.json + google-ads.json used to hold.
Those synthesised files were marked in their own _meta.note as
"Replace with live API for true paid-campaign data." This is that replacement.
...
# Support both:
#   - loaded as _lib.windsor_fetcher (relative import works)
#   - loaded standalone via importlib (relative import breaks; fall back)
try:
    from . import windsor_client as _w
except ImportError:
    import windsor_client as _w  # type: ignore
...
def _atomic_write(path: str, payload: dict) -> None:
    ...
    Used by both the CLI script (scripts/fetch_windsor.py) and the
    /api/admin/windsor-refresh endpoint. Keeping it here means the same
    guarantees (atomic, fsync, mode 0600) no matter where the write happens.
    """
    tmp = path + ".tmp"

...
const BASE='/Users/fivefriday/.openclaw-instance2/workspace/swing-shack-dashboard';const DATA=path.join(BASE,'data');const A='retargeting_campaign_builder';const start=Date.now();
let status='PASS',errMsg='';
try{execSync('node '+path.join(BASE,'scripts','run_'+A+'.js'),{cwd:BASE,timeout:30000});}catch(e){status='FAIL';errMsg=e.message.slice(0,80);}

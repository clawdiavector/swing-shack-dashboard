...
const BASE="/Users/fivefriday/.openclaw-instance2/workspace/swing-shack-dashboard";const DATA=path.join(BASE,"data");const A="autonomy_performance_reporter";const start=Date.now();
...
const r={agent_id:A,run_at:new Date().toISOString(),duration_ms:Date.now()-start,status,scripts:[{script:"run_"+A+".js",status,err:errMsg}],passed:status==="PASS"?1:0,failed:status==="FAIL"?1:0};
console.log("\\n["+A+"] "+status+" ("+r.duration_ms+"ms)");
